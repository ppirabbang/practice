...
- 2 ^ 32 를 2 ^ 16 ^ 2 로 나누면 연산 횟수가 줄어듦. 이걸 분할 정복이라고 하는데 블로그 참고.
- mod P 연산은 나눗셈에서는 적용되지 않음. 따라서 페르마의 소정리를 통한 역원을 구해야 되는데 P가 소수라고 했을 때 어떤 수 A ^ -1 과 A ^ P-2 의 mod P 연산값이 동일함을 사용해야 함
- row 라는 배열에서 print(row) 하면 [] 을 포함한 배열 그대로가 나오는데 print(*row) 를 하면 파이썬에서는 * 가 언패킹 연산자라 배열의 원소들만 나옴.
- print() 에는 default로 sep=' '가 설정되어 있어 알아서 띄어쓰기 돼서 나옴. sep="<" 로 설정하면 원소 사이가 < 로 출력됨.
