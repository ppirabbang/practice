...
- sort(key=lambda x : (x[1],x[0])) 이면 x 가 sort 진행 중인 배열의 한 원소를 가리키고 기준을 y좌표,x좌표로 잡으라는 것. 이러면 y 좌표가 동일할 때 x 좌표 크기에 맞게 정렬됨, x[0] 선언을 안하면 y좌표만 기준으로 잡기 때문에 y좌표가 같다면 입력 순서 그대로 놔둠
